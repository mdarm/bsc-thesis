% Τίτλος: \hspace{43pt}              EgregiousDataPadding.m
% Έκδοση:  \hspace{37pt}        1.3
% Συγγραφέας:   \hspace{14pt} Δαρμάνης Μιχαήλ
% Ημερομηνία:  \hspace{15pt}           12 Δεκεμβρίου 2021
% Περιγραφή:  \hspace{21pt}     Script για τη θερμοδυναμική ανάλυση των διατάξεων ροής.

%% Measured data and known values

tic;                          % χρονομέτρηση κώδικα

dInner = 22 * 10 ^ -3;        % διάμετρος εσωτερικού κυλίνδρου, [m]
dOuter = 40 * 10 ^ -3;        % διάμετρος εξωτερικού κυλίνδρου, [m]
voltRes = 29.5;               % τάση αντίστασης, [V]
ampRes = 0.63;                % ένταση ρεύματος αντίστασης, [A]
position = 0.05:0.1:0.9;      % θέσεις θερμοστοιχείων (z), [m]
...
timeData = zeros(1, 4, 17);   % χρόνος, [sec]
voltFan = zeros(1, 4, 17);    % τάση ανεμιστήρα, [V]
ampFan  = zeros(1, 4, 17);    % ένταση ρεύματος ανεμιστήρα, [A]
c = 0;
...
colorsErr = [140 45 4; 204 76 2;...
             236 112 20; 254 153 41;...
             99 99 99];    
...
for k = 1:16
    for i = 1:4
        ...
        ChangeInterpreter(gcf, 'Latex');
        Plot2LaTeX(gcf, ['temp', num2str(i)-num2str(k)]);
...
relData = table(name, round(relmean, 2), round(lci, 2), round(uci, 2), 'Variablenames', colNames);
...
mu = @(T) 1.716e-5 * ((T + 273.15) / 273.15) ^ (3 / 2) * ((273.15 + 110.56) / (T + 110.56 + 273.15));
...
h = @(voltRes, ampRes, dInner, Tres, Tair) voltRes * ampRes / (pi * dInner * 0.1 * (Tres - Tair));
...
for k = 1:17
    for i = 1:4
        [flowrate, uflowrate] = UncertaintyPropagation(flowFunc, timeData(1, i, k), uTime);
        
        % Θερμοδυναμικές ιδιότητες αέρα για Tavg
        tfunc = @(Tin, Tout) (Tin + Tout) / 2;
        [Tavg, uTavg] = UncertaintyPropagation(tfunc, [tempData(10, i, k) tempData(11, i, k)], [uTemp(10, 1) uTemp(11, 1)]);
        ...
        % Θερμική ενέργεια
        [thermalEn(1, i, k), thermalEnErr(1, i, k)] = UncertaintyPropagation(Qen, [density flowrate Cp tempData(11, i, k) tempData(10, i, k)], [udensity uflowrate uCp uTemp(11, 1) uTemp(10, 1)]);
        ...
        [const, uconst] = LeastSquaresFit([0 0.9], [tempData(10, i, k) tempData(11, i, k)], [0 0], [uTemp(10, 1) uTemp(11, 1)], 'linear');
        ...
        for j =1:9
            ...
            [htc, uhtc] = UncertaintyPropagation(h, [voltRes ampRes dInner Tres Tair(j)], [uVolt uAmp uDim uTres uTair(j)]);
            
            % Τοπικός Nusselt
            [Nusselt(j, i, k), NusseltErr(j, i, k)] = UncertaintyPropagation(Nu, [htc dOuter dInner tcond], [uhtc uDim uDim utcond]);
        end
        % Μέσοι Nusselt
        [wmean, wvar, wstd, weights] = WeightedVariance(Nusselt(:, i, k), NusseltErr(:, i, k));
    ...
    [cc, cerrr, statt] = LeastSquaresFit(reynolds, nusselt, reynoldserr, nusselterr, 'power');
    ...
    if k <= 4
        a = k;
        handles(1).hE{k} = errorbar(handles(1).hAxes{1}, reynolds, nusselt, nusselterr, nusselterr, reynoldserr, reynoldserr, '*', 'Color', colorsErr(a, :) / 255);
        handles(1).hP{k} = plot(handles(1).hAxes{1}, rFit, nFit, '-', 'Color', colorsFit(a, :) / 255);
    elseif (k > 4) && (k <= 8)
        a = k - 4;
        handles(1).hE{k} = errorbar(handles(1).hAxes{2}, reynolds, nusselt, nusselterr, nusselterr, reynoldserr, reynoldserr, '*', 'Color', colorsErr(a, :) / 255);
        handles(1).hP{k} = plot(handles(1).hAxes{2}, rFit, nFit, '-', 'Color', colorsFit(a, :) / 255);           
    elseif (k > 8) && (k <= 12)
        a = k - 8;
        handles(1).hE{k} = errorbar(handles(1).hAxes{3}, reynolds, nusselt, nusselterr, nusselterr, reynoldserr, reynoldserr, '*', 'Color', colorsErr(a, :) / 255);
        handles(1).hP{k} = plot(handles(1).hAxes{3}, rFit, nFit, '-', 'Color', colorsFit(a, :) / 255); 
    elseif (k > 12) && (k <= 16)
        a = k - 12;
        handles(1).hE{k} = errorbar(handles(1).hAxes{4}, reynolds, nusselt, nusselterr, nusselterr, reynoldserr, reynoldserr, '*', 'Color', colorsErr(a, :) / 255);
        handles(1).hP{k} = plot(handles(1).hAxes{4}, rFit, nFit, '-', 'Color', colorsFit(a, :) / 255); 
    else
        for i = 1:4
            a = 16 + i;
            handles(1).hE{a} = errorbar(handles(1).hAxes{i}, reynolds, nusselt, nusselterr, nusselterr, reynoldserr, reynoldserr, '*', 'Color', colorsErr(5, :) / 255);
            handles(1).hP{a} = plot(handles(1).hAxes{i}, rFit, nFit, '-', 'Color', colorsFit(5, :) / 255);
        end
    end
 
    % Αποθήκευση δεδομένων παρεμβολών
    stats(1).c(k, :) = cc'; stats1.cerr(k, :) = cerrr';
...
% Μέσοι αριθμοί Nusselt και οι αβεβαιότητες τους
...
for i = 1:16
    [relswirlNu(i, 1), relswirlNu(i, 2)] = UncertaintyPropagation(relpower, [swirlNu(i, 1) axialNu(1, 1)], [swirlNu(i, 2) axialNu(1, 2)]);
end 
...
nuss = [relswirlNu(1:4, 1)'; relswirlNu(5:8, 1)'; relswirlNu(9:12, 1)'; relswirlNu(13:16, 1)'];
nusserr = [relswirlNu(1:4, 2)'; relswirlNu(5:8, 2)'; relswirlNu(9:12, 2)'; relswirlNu(13:16, 2)'];

bar(handles(1).hAxes{5}, nuss);
ngroups = size(nuss, 1);
nbars = size(nuss, 2);

% Υπολογισμός πλάτους κάθε γραφήματος μπάρας
groupwidth = min(0.8, nbars/(nbars + 1.5));
for i = 1:nbars
    x = (1:ngroups) - groupwidth/2 + (2*i-1) * groupwidth / (2*nbars);
    handles(1).hPe{i} = errorbar(handles(1).hAxes{5}, x, nuss(:, i), nusserr(:, i), '.');
end
hold off

xlabel(handles(1).hAxes{5}, 'Inlet angle');
ylabel(handles(1).hAxes{5}, 'Relative thermal performance ($\%$)');
legend(handles(1).hAxes{5}, {'1 inlet', '2 inlets', '3 inlets', '4 inlets'}, 'orientation', 'horizontal', 'Location', 'Northoutside');
      
colororder(colorsFit(1:4, 1:3) / 255);
set(handles(1).hAxes{5}, 'XTick', 1:4, 'XTickLabel', {'45$^o$', '60$^o$', '75$^o$', '90$^o$'});
...
Plot2LaTeX(handles(1).hFig{5}, 'thermal_performance');
...
for k = 1:17
    for i = 1:4
        [wattFan(1, i, k), wattFanErr(1, i, k)] = UncertaintyPropagation(wattFunc, [voltFan(1, i, k) ampFan(1, i, k)], [uVolt uAmp]);
        [flowFan(1, i, k), flowFanErr(1, i, k)] = UncertaintyPropagation(flowFunc, timeData(1, i, k), uTime);
    end
    ...
    [cc, cerrr, statt] = LeastSquaresFit(flowdata, wattdata, flowerr, watterr, 'power');
    ...
    if k <= 4
        a = k;
        handles(2).hE{k} = errorbar(handles(2).hAxes{1}, flowdata, wattdata, watterr, watterr, flowerr, flowerr, '*', 'Color', colorsErr(a, :) / 255);
        handles(2).hP{k} = plot(handles(2).hAxes{1}, qFit, pFit, '-', 'Color', colorsFit(a, :) / 255);
    elseif (k > 4) && (k <= 8)
        a = k - 4;
        handles(2).hE{k} = errorbar(handles(2).hAxes{2}, flowdata, wattdata, watterr, watterr, flowerr, flowerr, '*', 'Color', colorsErr(a, :) / 255);
        handles(2).hP{k} = plot(handles(2).hAxes{2}, qFit, pFit, '-', 'Color', colorsFit(a, :) / 255);             
    elseif (k > 8) && (k <= 12)
        a = k - 8;
        handles(2).hE{k} = errorbar(handles(2).hAxes{3}, flowdata, wattdata, watterr, watterr, flowerr, flowerr, '*', 'Color', colorsErr(a, :) / 255);
        handles(2).hP{k} = plot(handles(2).hAxes{3}, qFit, pFit, '-', 'Color', colorsFit(a, :) / 255);    
    elseif (k > 12) && (k <= 16)
        a = k - 12;
        handles(2).hE{k} = errorbar(handles(2).hAxes{4}, flowdata, wattdata, watterr, watterr, flowerr, flowerr, '*', 'Color', colorsErr(a, :) / 255);
        handles(2).hP{k} = plot(handles(2).hAxes{4}, qFit, pFit, '-', 'Color', colorsFit(a, :) / 255);
    else
        for i = 1:4
            a = 16 + i;
            handles(2).hE{a} = errorbar(handles(2).hAxes{i}, flowdata, wattdata, watterr, watterr, flowerr, flowerr, '*', 'Color', colorsErr(5, :) / 255);
            handles(2).hP{a} = plot(handles(2).hAxes{i}, qFit, pFit, '-', 'Color', colorsFit(5, :) / 255);
        end
    end
...
% Μέσες τιμές ισχύος και οι αντίστοιχες αβεβαιότητες
pwavg(:, 1) = relWattFan(1, 1, 1:16); 
pwavg(:, 2) = relWattFanErr(1, 1, 1:16);

% Ποσοστιαία καταναλισκόμενη ισχύς διατάξεων, συγκριτικά με την
% αντίστοιχη της διάταξης αξονικής ροής
...
relpower = @(swirlpower, axialpower) (swirlpower / axialpower - 1) * 100;
for i = 1:16
    [relswirlpower(i, 1), relswirlpower(i, 2)] = UncertaintyPropagation(relpower, [swirlpower(i, 1) axialpower(1, 1)], [swirlpower(i, 2) axialpower(1, 2)]);
end 
...
power = [relswirlpower(1:4, 1)'; relswirlpower(5:8, 1)'; relswirlpower(9:12, 1)'; relswirlpower(13:16, 1)'];
powererr = [relswirlpower(1:4, 2)'; relswirlpower(5:8, 2)'; relswirlpower(9:12, 2)'; relswirlpower(13:16, 2)'];
...
% Υπολογισμός πλάτους κάθε γραφήματος μπάρας
% (αυτό γίνεται για την τοποθέτηση της συνοδευόμενης αβεβαιότητας)
groupwidth = min(0.8, nbars / (nbars + 1.5));
for i = 1:nbars
    x = (1:ngroups) - groupwidth/2 + (2*i-1) * groupwidth / (2*nbars);
...
xlabel(handles(2).hAxes{5}, 'Inlet angle');
ylabel(handles(2).hAxes{5}, 'Relative power consumption ($\%$)');
legend(handles(2).hAxes{5}, {'1 inlet', '2 inlets', '3 inlets', '4 inlets'}, 'orientation', 'horizontal', 'Location', 'Northoutside');
      
colororder(colorsFit(1:4, 1:3) / 255);
set(handles(2).hAxes{5}, 'XTick', 1:4, 'XTickLabel', {'45$^o$', '60$^o$', '75$^o$', '90$^o$'});
...
Plot2LaTeX(handles(1).hFig{5}, 'power_performance');

%% Publication quality graphics and aesthetic considerations

n_hE = length(handles(2).hE);
for i = 1:n_hE
   for k = 1:2
       set(handles(k).hE{i}, 'Capsize', 0, 'LineWidth', 0.6, 'MarkerSize', 4);
...
for i = 1:n_hFig
    for k = 1:2
        ...
        set(handles(k).hAxes{i}, 'XLim', hor_lim{k}, 'YLim', vert_lim{k}, 'Box', 'off', 'XMinorTick', 'on', 'YMinorTick', 'on', 'TickDir', 'out', 'TickLength', [.02 .02], 'LineWidth', 1, 'XColor', [.3 .3 .3], 'YColor', [.3 .3 .3], 'YGrid', 'on');
        
        % Υπόμνημα
        legend(handles(k).hAxes{i}, [handles(k).hE{1}, handles(k).hP{17}, handles(2).hP{1:4}], 'Data ({$\it\mu$} $\pm$ {$\it\sigma$})', 'axial flow', '1 inlet', '2 inlets', '3 inlets', '4 inlets', 'Location', 'NorthOutside', 'Orientation', 'horizontal', 'NumColumns', 3);
        ...
        Plot2LaTeX(handles(k).hFig{i}, name{k});
    end
end

%% Potential efficiency index

% Ωφέλιμο δυναμικό διατάξεων, συγκριτικά με το αντίστοιχο της αξονικής
% ροής
...
for k = 1:17
   for i = 1:4
       [potentialEn(1, i, k), potentialEnErr(1, i, k)] = UncertaintyPropagation(pee, [thermalEn(1, i, k) wattFan(1, i, k)], [thermalEnErr(1, i, k) wattFanErr(1, i, k)]);
   end
end 
...
for i = 1:16
    [relswirlTh(i, 1), relswirlTh(i, 2)] = UncertaintyPropagation(relthermal, [swirlTh(i, 1) axialTh(1, 1)], [swirlTh(i, 2) axialTh(1, 2)]);
end 
...
therm = [relswirlTh(1:4, 1)'; relswirlTh(5:8, 1)'; relswirlTh(9:12, 1)'; relswirlTh(13:16, 1)'];
thermerr = [relswirlTh(1:4, 2)'; relswirlTh(5:8, 2)'; relswirlTh(9:12, 2)'; relswirlTh(13:16, 2)'];
...
for i = 1:nbars
    x = (1:ngroups) - groupwidth/2 + (2*i - 1) * groupwidth / (2*nbars);
    handles(1).hPe{i} = errorbar(handles(1).hAxes{6}, x, therm(:, i), thermerr(:, i), '.');
end
hold off

xlabel(handles(1).hAxes{6}, 'Inlet angle');
ylabel(handles(1).hAxes{6}, 'Potential energy efficiency ($\%$)');
legend(handles(1).hAxes{6}, {'1 inlet', '2 inlets', '3 inlets', '4 inlets'}, 'orientation', 'horizontal', 'Location', 'Northoutside');
      
colororder(colorsFit(1:4, 1:3) / 255);
set(handles(1).hAxes{6}, 'XTick', 1:4, 'XTickLabel', {'45$^o$', '60$^o$', '75$^o$', '90$^o$'});
...
Plot2LaTeX(handles(1).hFig{6}, 'potential_energy_efficiency');
...
% Σατιστικά χαρακτηριστικών καμπυλών παρεμβολής
...
% Οργάνωση δεδομένων Nusslet και Ισχύος
...
% Δείκτης βελτίωσης καταναλισκόμενης ισχύος
